...
// SlicePool provides reusable []interface{} slices.
...
// AcquireSlice gets a []interface{} slice from the pool.
func AcquireSlice() *[]interface{} {
	return SlicePool.Get().(*[]interface{})
}

// ReleaseSlice clears and returns a []interface{} slice to the pool.
...
// MapSlicePool provides reusable []map[string]interface{} slices.
...
// AcquireMapSlice gets a []map[string]interface{} slice from the pool.
func AcquireMapSlice() *[]map[string]interface{} {
	return MapSlicePool.Get().(*[]map[string]interface{})
}

// ReleaseMapSlice clears and returns a []map[string]interface{} slice to the pool.
...
// LargeByteBufferPool provides reusable 16KB bytes.Buffer instances.
...
// AcquireLargeByteBuffer gets a 16KB bytes.Buffer from the pool.
func AcquireLargeByteBuffer() *bytes.Buffer {
	return LargeByteBufferPool.Get().(*bytes.Buffer)
}

// ReleaseLargeByteBuffer resets and returns a 16KB bytes.Buffer to the pool.
func ReleaseLargeByteBuffer(b *bytes.Buffer) {
	if b == nil {
		return
	}
	b.Reset()
	LargeByteBufferPool.Put(b)
}
